...
adminclient = Flask(__name__)

@adminclient.route("/login", methods=["GET"])
def get_loginMethod():
    return render_template("login.html")

@adminclient.route("/login", methods=["POST"])
...
@adminclient.route("/categories", methods=["GET"])
def get_categories():
	categ_id = ""
	categ_name = ""

	r = requests.get("http://adminserver:1000/get_categ").json()
	...
	if categ_id is None:
		...
		if delete_id != "":
			ret = requests.post("http://adminserver:1000/delete_categ", json=payload).json()
			r = requests.get("http://adminserver:1000/get_categ").json()
			categories = r['categories']
			return render_template("categories.html", categories=categories, categ_id="", categ_name="")
	payload = {"categ_id":categ_id}
	if categ_id != "":
		ret = requests.post("http://adminserver:1000/get_categ_by_id", json=payload).json()
...
@adminclient.route("/categories", methods=["POST"])
def post_categories():
	...
	if categ_name != "":
		if funct_id == "":
			...
			ret = requests.post("http://adminserver:1000/add_categ", json=payload).json()
		else:
			payload = {"id": funct_id, "categ_name": categ_name}
			ret = requests.post("http://adminserver:1000/update_categ", json=payload).json()
	
	r = requests.get("http://adminserver:1000/get_categ").json()
	categories = r['categories']
	return render_template("categories.html", categories=categories, categ_id="", categ_name="")

@adminclient.route("/products", methods=["GET"])
def get_products():
	r = requests.get("http://adminserver:1000/get_categ").json()
	categories = r['categories']
	r = requests.get("http://adminserver:1000/get_products").json()
	products = r['products']
	...
	if prod_id is None:
		...
		if delete_id != "":
			ret = requests.post("http://adminserver:1000/delete_product", json=payload).json()
			r = requests.get("http://adminserver:1000/get_categ").json()
			categories = r['categories']
			r = requests.get("http://adminserver:1000/get_products").json()
			products = r['products']
	...
	if prod_id != "":
		ret = requests.post("http://adminserver:1000/get_products_by_id", json=payload).json()
...
@adminclient.route("/products", methods=["POST"])
def post_products():
	...
	if prod_name != "" and brand != "" and price != "" and stock != "":
		if funct_id == "":
			...
			ret = requests.post("http://adminserver:1000/add_product", json=payload).json()
		else:
			payload = {"prod_id": funct_id, "categ_id": categ_id, "prod_name": prod_name, "brand": brand, "descr": descr, "pict": pict, "price": price, "stock": stock}
			ret = requests.post("http://adminserver:1000/update_product", json=payload).json()
...
@adminclient.route("/orders", methods=["GET"])
def get_orders():
	ret = requests.get("http://adminserver:1000/get_orders").json()
	return render_template("orders.html", orders=ret['orders'])

if __name__ == '__main__':
	adminclient.run(host='0.0.0.0',port='2000',debug=True)